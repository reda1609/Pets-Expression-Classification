        ...
        if csv_logger_instance:
            ...
            if os.path.exists(metrics_file_path):
                try:
                    ...
                    plotable_metrics = ["val_loss", "val_acc", "val_precision", "val_recall", "val_f1", "train_loss_epoch", "train_accuracy_epoch"]
                    
                    if "train_loss_step" in metrics_df.columns and "train_loss_epoch" not in metrics_df.columns:
                        metrics_df.rename(columns={"train_loss_step": "train_loss_epoch"}, inplace=True)
                    if "train_accuracy_step" in metrics_df.columns and "train_accuracy_epoch" not in metrics_df.columns:
                        metrics_df.rename(columns={"train_accuracy_step": "train_accuracy_epoch"}, inplace=True)
...
def main(args):
    ...
    # --- Logger Setup ---
    csv_logger = CSVLogger(save_dir="logs", name=model_name_arg)
    tensorboard_logger = TensorBoardLogger(save_dir="logs", name=model_name_arg, sub_dir="tensorboard_events")
